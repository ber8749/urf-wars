...
export class MechModel {
  ...
  private createTorso(): THREE.Group {
    ...
    const reactor = new THREE.Mesh(reactorGeom, this.materials.get('armorSecondary'));
    ...
    for (let i = -1; i <= 1; i += 2) {
      const shoulder = new THREE.Mesh(shoulderGeom, this.materials.get('armorSecondary'));
  ...
  private createLeg(side: 'left' | 'right'): THREE.Group {
    ...
    const plate = new THREE.Mesh(plateGeom, this.materials.get('armorSecondary'));
  ...
  animateWalk(time: number, speed: number): void {
    ...
    const bobAmount = Math.abs(Math.sin(walkCycle * 2)) * 0.1 * Math.min(speed, 1);
    ...
    // Slight torso sway
    this.torsoGroup.rotation.z = Math.sin(walkCycle) * 0.02 * Math.min(speed, 1);
    
    // Arm counter-swing
    this.leftArmGroup.rotation.x = -legSwing * 0.5;
    this.rightArmGroup.rotation.x = legSwing * 0.5;
  }
